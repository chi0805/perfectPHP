...
class AccountController extends Controller {
   ...
   public function registerAction() {
        ...
        //入力アクションでのトークンの埋め込みと、処理アクションでのCSRFトークンに関する記述はセットで行う。
        $token = $this->request->getPost('_token');
        if (!$this->checkCsrfToken('account/signup', $token)) {
            //処理を中断するため、returnを用いてリダイレクトする。
        ...
        if(count($errors) == 0) {
            ...
            $user = $this->db_manager->get('User')->fetchByUserName($user_name); //主キーであるidカラムに連番がふられる。
            $this->session->set('user', $user);
    
            return $this->redirect('/'); //レコードの登録と取得が完了後、ユーザーのホームページへリダイレクト。処理を中断する。
        }
